...
# TTF to WOFF
def TTFtoWOFF(filename):
    f = TTFont(filename)
    f.flavor = 'woff'
    f.save(str(os.path.splitext(filename)[0]) +'.woff')

# TTF to WOFF2
def TTFtoWOFF2(filename):
    f = TTFont(filename)
    f.flavor = 'woff2'
    f.save(str(os.path.splitext(filename)[0]) +'.woff2')

# WOFF/WOFF2 to TTF
def WOFFtoTTF(filename):
    f = TTFont(filename)
    f.flavor = None
    f.save(str(os.path.splitext(filename)[0]) +'.ttf')
    newTTFname = str(os.path.splitext(filename)[0]) +'.ttf'
    return newTTFname
